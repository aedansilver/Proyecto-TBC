...
struct TRINITY_DLL_DECL boss_strawmanAI : public ScriptedAI
{
    ...
    {
        if((Spell->SchoolMask == SPELL_SCHOOL_MASK_FIRE) && caster->GetTypeId() == TYPEID_PLAYER)
    ...
    void UpdateAI(const uint32 diff)
    {
        if(AggroTimer)
        {
            if(AggroTimer <= diff)
            {
                m_creature->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                AggroTimer = 0;
				Unit* Dorothee = GetClosestCreatureWithEntry(m_creature, 17535, 50);
				if(Dorothee && Dorothee->isAlive() && Dorothee->getVictim())
					m_creature->AI()->AttackStart(Dorothee->getVictim());
            }else AggroTimer -= diff;
        }
