...
struct TRINITY_DLL_DECL Mob_EventAI : public ScriptedAI
{
    ...
    void ProcessAction(uint16 type, uint32 param1, uint32 param2, uint32 param3, uint32 rnd, uint32 EventId, Unit* pActionInvoker)
    {
        ...
        case ACTION_T_FLEE:
            {
                if(m_creature->HasAuraType(SPELL_AURA_PREVENTS_FLEEING)||!m_creature->CanFreeMove())
                    break;
                TimetoFleeLeft = 8000;
        ...
        case ACTION_T_ZONE_COMBAT_PULSE:
            ...
            break;
        case ACTION_T_DESPAWN:
        {
             if (m_creature->isDead())
                {
                    if (EAI_ErrorLevel > 0)
                        error_db_log("TSCR: Event %d ACTION_T_DESPAWN on dead creature. Creature %d", EventId, m_creature->GetEntry());
                    return;
                }
                m_creature->DealDamage(m_creature, m_creature->GetMaxHealth(),NULL, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, NULL, false);
