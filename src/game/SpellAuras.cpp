...
,m_tickNumber(0)
{
	...
	int32 tickTime = m_spellProto->EffectAmplitude[eff];
	// Channeled spells are hasted
    if(caster && !(m_spellProto->Attributes & (SPELL_ATTR_UNK4|SPELL_ATTR_UNK5)) && IsChanneledSpell(m_spellProto))
        tickTime = int32(tickTime * caster->GetFloatValue(UNIT_MOD_CAST_SPEED));

    SetModifier(AuraType(m_spellProto->EffectApplyAuraName[eff]), damage, tickTime, m_spellProto->EffectMiscValue[eff]);
...
//                    case 33563: break;
					...
					{
						if(!m_duration)
						m_target->CastSpell(m_target,38794,true);
						return;
					}
                    // Flame Quills
                    case 34229:
    ...
    if(apply)
    {
        switch(GetId())
        {
            ...
            case 39850:                                     // Rocket Blast
				...
				break;
					m_target->CastSpell(m_target, 8413, true, NULL, this);	// for breaking cc
                return;
    ...
    switch(m_spellProto->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_DRUID:
        {
            ...
            if ( GetSpellProto()->SpellFamilyFlags & 0x1000000000LL )
            {
                ...
                // Do final when not stacking
                else if (m_removeMode != AURA_REMOVE_BY_STACK)
                {
...
void Aura::HandleAuraModDecreaseSpeed(bool /*apply*/, bool Real)
{
	...
	switch(m_spellProto->Id)
	{
		case 12323: // Durchdringendes Heulen
		case 18223: // Fluch der Erschöpfung
